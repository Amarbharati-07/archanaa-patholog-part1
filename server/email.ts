...
export async function sendOtpEmail(to: string, otp: string, purpose: string): Promise<boolean> {
  ...
  setImmediate(async () => {
    try {
      ...
      console.log(`📧 [SENDOTP] Attempting to send from: ${sender} to: ${to}`);
  ...
  const content = `
    <div class="content">
      ...
      <div class="info-box">
        ...
        <div class="info-row"><span class="info-label">Total Amount</span><span class="info-value" style="color: #005B96; font-size: 16px;">Rs. ${totalAmount.toFixed(2)}</span></div>
  ...
  const content = `
    <div class="content">
      ...
      <div class="info-box">
        ...
        <div class="info-row"><span class="info-label">Amount</span><span class="info-value" style="color: #28a745; font-weight: 600;">Rs. ${amount.toFixed(2)}</span></div>
